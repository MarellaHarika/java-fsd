...
class Main{  
 public static void main(String args[]){  
    ...
    int first = 0;
    //set last to last elements in the array
    int last=numArray.length-1; 
    //calculate mid of the array
    int mid = (first + last)/2;  
    //while first and last do not overlap
    while( first <= last ){  
        //if the mid < key, then the key to be searched is in the first half of the array
        if ( numArray[mid] < key ){  
            first = mid + 1;     
   ...
   //if the first and last overlap, then the key is not present in the array
   if ( first > last ){  
